...
adresu urządzenia na magistrali \isqrc, pole przechowujące dane
kompensacyjne pomiaru oraz parametr typu \texttt{CONFIG} zawarty
...
Dane kompensacyjne zapisywane są do struktury
\texttt{Bmp280Compensations}, składającej się z~dwunastu
szesnastobitowych pól przechowujących wartości kompensacyjne.
Są one wykorzystywane przy przeliczaniu surowych danych odczytanych
z~modułu na temperaturę w~stopniach Celsjusza oraz ciśnienie
w~paskalach. Implementacja struktury udostępnia funkcję powiązaną
\texttt{read\_from\_i2c}, odczytującą 24~bajty z~pamięci modułu
BMP280 i~przypisującą je do poszczególnych pól struktury.
Implementacja tej funkcji przedstawiona jest na
listingu~\ref{lst:bmp280compensations_read}, a~nazwy pól struktury
przyjęto zgodnie z~notą katalogową~\cite{bmp280_datasheet}.
...
i~\texttt{IIRConstant} zostały pominięte, ponieważ są już widoczne
na listingu~\ref{lst:bmp280_config}.
